...
func newMovers(rootSDK *SDK, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Movers {
	return &Movers{
		rootSDK:          rootSDK,
...
func (s *Movers) GetMovers(ctx context.Context, symbolID components.PathParamSymbol, sort *components.QueryParamSort, frequency *components.QueryParamFrequency, opts ...operations.Option) (*operations.GetMoversResponse, error) {
	request := operations.GetMoversRequest{
		SymbolID:  symbolID,
		Sort:      sort,
		Frequency: frequency,
	}
	...
	baseURL := utils.ReplaceParameters(operations.GetMoversServerList[0], map[string]string{})
	if o.ServerURL != nil {
		baseURL = *o.ServerURL
	}

	opURL, err := utils.GenerateURL(ctx, baseURL, "/movers/{symbol_id}", request, nil)
	if err != nil {
	...
	res := &operations.GetMoversResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out operations.GetMoversResponseBody
