...
type OptionExpirationChain struct {
	rootSDK          *SDK
	sdkConfiguration config.SDKConfiguration
	hooks            *hooks.Hooks
}

func newOptionExpirationChain(rootSDK *SDK, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *OptionExpirationChain {
	return &OptionExpirationChain{
		rootSDK:          rootSDK,
...
func (s *OptionExpirationChain) GetExpirationChain(ctx context.Context, symbol string, opts ...operations.Option) (*operations.GetExpirationChainResponse, error) {
	request := operations.GetExpirationChainRequest{
		Symbol: symbol,
	}
	...
	baseURL := utils.ReplaceParameters(operations.GetExpirationChainServerList[0], map[string]string{})
	if o.ServerURL != nil {
		baseURL = *o.ServerURL
	}

	opURL, err := url.JoinPath(baseURL, "/expirationchain")
	if err != nil {
	...
	res := &operations.GetExpirationChainResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.ExpirationChain
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.ExpirationChain = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
