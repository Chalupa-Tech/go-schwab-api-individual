...
func newMarketHours(rootSDK *SDK, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *MarketHours {
	return &MarketHours{
		rootSDK:          rootSDK,
...
func (s *MarketHours) GetMarketHours(ctx context.Context, markets []components.QueryParamMarkets, date *types.Date, opts ...operations.Option) (*operations.GetMarketHoursResponse, error) {
	request := operations.GetMarketHoursRequest{
		Markets: markets,
		Date:    date,
	}
	...
	baseURL := utils.ReplaceParameters(operations.GetMarketHoursServerList[0], map[string]string{})
	if o.ServerURL != nil {
		baseURL = *o.ServerURL
	}

	opURL, err := url.JoinPath(baseURL, "/markets")
	if err != nil {
	...
	res := &operations.GetMarketHoursResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out map[string]map[string]components.Hours
...
func (s *MarketHours) GetMarketHour(ctx context.Context, marketID components.PathParamMarket, date *types.Date, opts ...operations.Option) (*operations.GetMarketHourResponse, error) {
	request := operations.GetMarketHourRequest{
		MarketID: marketID,
		Date:     date,
	}
	...
	baseURL := utils.ReplaceParameters(operations.GetMarketHourServerList[0], map[string]string{})
	if o.ServerURL != nil {
		baseURL = *o.ServerURL
	}

	opURL, err := utils.GenerateURL(ctx, baseURL, "/markets/{market_id}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.GetMarketHourResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
